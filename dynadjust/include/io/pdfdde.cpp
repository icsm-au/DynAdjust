...
PURPOSE
	...
	the viewer to open a file called "test.pdf" that is located at
	the root level of the c drive.  The viewer must be running in
...
bool __cdecl ExecuteCommand(LPCTSTR command, ...)
{
    HDDEDATA ret;
    va_list args;
    int len;
...
void usage()
{
  ...
  fprintf(stderr, "\tpdfopen  --file <filename> [--page <page>] [--back]\n");
  fprintf(stderr, "\t\t       Opens <filename> (at page <page> if available)\n");
  fprintf(stderr, "\t\t                        (optionally go back to previous view)\n");
...
int pdfopen()
{
  ...
  ExecuteCommand("[DocOpen(\"%s\")]", sLongFileName); /* acrord32 ver8 bug */
...
#define WHITE_DELIM " \t\n"
    ...
    if (stricmp(p, "open") == 0) {
      ...
      fprintf(stdout, "back                     goto the previous view\n");
...
int main (int argc, char *argv[])
{
  ...
  for(;;) {
    g = my_getopt_long_only (argc, argv, "", long_options, &option_index);
    ...
    */
	...
	fprintf(stderr, "%s: warning, `%s' bad debug argument.\n", argv0, optarg);
      ...
      if (ARGUMENT_IS("file")) {
	CheckFileName(my_optarg);
      }
      else if (ARGUMENT_IS("page")) {
	nPage = atoi(my_optarg) - 1;
      }
      else if (ARGUMENT_IS("goto")) {
	sGoto = my_optarg;
      }
      else if (ARGUMENT_IS("back")) {
	bBack = TRUE;
      }
    }
