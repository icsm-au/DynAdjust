...
string dna_io_snx::parse_date_from_yy_doy(const UINT32& yy, const UINT32& doy, DATE_FORMAT_TYPE date_type, const string& separator)
{
...
string dna_io_snx::parse_date_from_string(const string& date_str, DATE_FORMAT_TYPE date_type, DATE_TERMINAL_TYPE terminal_type, const string& separator)
{
	// Year
...
void dna_io_snx::parse_sinex_epochs(std::ifstream** snx_file, UINT32& lineNo, UINT32& columnNo)
{
	...
	while (sBuf.at(0) != '-')
	{
		...
		try {
			...
			date_start = parse_date_from_string(trimstr(sBuf.substr(16, 6)), doy_yyyy, date_from, string(" "));
		}
		catch (...) {
			ss.str("");
			columnNo = 1;
